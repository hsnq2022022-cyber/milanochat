...
export async function extractFromUrl(url: string): Promise<{ text: string; title: string }> {
  ...
  if (type.includes("text/plain") || type.includes("json")) {
    return { title: url, text: raw.slice(0, MAX_CHARS) };
  }
  ...
  const root = $("main").first().text() || $("article").first().text() || $("body").text();
  const text = root
    .replace(/\s+/g, " ")
    .replace(/\s*([،؛:.,!?])\s*/g, "$1 ")
    .trim()
...
export function chunkText(text: string, size = 700, overlap = 120): string[] {
  ...
  let buf = "";
  const push = () => {
    if (buf.trim().length > 30) chunks.push(buf.trim());
    buf = "";
  };
  ...
  for (const c of chunks) {
    if (c.length <= size + 300) final.push(c);
    else for (let i = 0; i < c.length; i += size - overlap) final.push(c.slice(i, i + size));
  }
  return final;
}
...
export async function ingestSource(tenantId: string, input: IngestInput): Promise<IngestResult> {
  ...
  try {
    ...
    if (raw.length < 40) {
      await db.from("knowledge_sources").update({ status: "failed", error: "المحتوى المستخرج غير كافٍ" }).eq("id", source.id);
      return { sourceId: source.id, status: "failed", chunks: 0, error: "المحتوى المستخرج غير كافٍ" };
    }
    ...
    await db
      .from("knowledge_sources")
      .update({ status: "indexed", chunks_count: chunks.length })
      .eq("id", source.id);

    return { sourceId: source.id, status: "indexed", chunks: chunks.length, title };
  } catch (e: any) {
    const msg = String(e?.message ?? e);
    await db.from("knowledge_sources").update({ status: "failed", error: msg }).eq("id", source.id);
    return { sourceId: source.id, status: "failed", chunks: 0, error: msg };
  }
}

/** إضافة قطعة معرفة واحدة (تُستخدم عند حل سؤال عالق من اللوحة) */
export async function addKnowledgeSnippet(tenantId: string, content: string): Promise<void> {
