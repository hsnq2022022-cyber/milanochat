...
const normalizeDirection = (direction: unknown): "in" | "out" =>
  direction === "out" || direction === "outbound" ? "out" : "in";
...
export default function Dashboard() {
      ...
      setSt((prev) => ({
        ...
        convs: skipConvs ? (prev?.convs || []) : convs.map((c: any) => {
          ...
          return {
            ...
            lastMessagePreview: c.lastMessageBody?.trim() || "",
            msgs: [],
          };
        }),
  ...
  /* ── أدوات مزامنة المحادثات والرسائل ── */
  const refreshConversations = useCallback(async () => {
    if (!token) return;
    try {
      const convs = await apiAuthFetch<any[]>(token, "/api/dashboard/conversations");
      setSt((prev) => {
        if (!prev) return prev;

        const byId = new Map(prev.convs.map((c) => [c.id, c]));
        const next = convs.map((c: any) => {
          const old = byId.get(c.id);
          const nowDate = new Date();
          const expiresAt = c.humanAgentExpiresAt ? new Date(c.humanAgentExpiresAt) : null;
          const humanAgentActive = Boolean(c.transferred && expiresAt && expiresAt > nowDate);
          const remainingSeconds = humanAgentActive && expiresAt
            ? Math.max(0, Math.floor((expiresAt.getTime() - nowDate.getTime()) / 1000))
            : 0;
          ...
          return {
            ...
            humanAgentActive,
            remainingSeconds,
            lastAt: c.lastMessageAt || old?.lastAt || new Date(0).toISOString(),
            lastMessagePreview: c.lastMessageBody?.trim() || old?.lastMessagePreview || "",
            msgs: old?.msgs ?? [],
          } as ConvItem;
        });
      ...
      console.error("[Dashboard] refreshConversations error:", e);
    }
  }, [token]);
  ...
  /* تحميل الرسائل مرة واحدة عند فتح المحادثة — لا polling */
  useEffect(() => {
    if (demo || !activeConv || !token) return;
    loadThread(activeConv).catch(() => {});
  }, [demo, activeConv, token, loadThread]);

  /* ── Supabase Realtime: مصدر واحد للحالة الحية ── */
  useEffect(() => {
    ...
    const messagesChannel = sb
      ...
      .on(
        ...
        (payload) => {
          ...
          /*
           * body_encrypted القادم من Realtime لا يمكن للمتصفح فك تشفيره.
           * لذلك نستخدم الحدث فقط كإشارة فورية، ثم نجلب البيانات المفكوكة
           * من endpoint المحمي. هذا ليس polling: لا يحدث إلا عند INSERT فعلي.
           */
          setSt((prev) => {
            if (!prev) return prev;
            const isIncoming = normalizeDirection(newMsg.direction) === "in";
            if (!isIncoming || activeConv === convId) return prev;
            return {
              ...prev,
              convs: prev.convs.map((c) =>
                c.id === convId
  ...
  }, [demo, token, needClaim, sb, activeConv, refreshConversations, loadThread]);
  ...
  const replyManual = async () => {
    ...
    try {
      ...
      const result = await apiAuthFetch<any>(
        token!,
        `/api/dashboard/conversations/${activeConv}/reply`,
        {
          method: "POST",
          body: JSON.stringify({ text, resumeAuto }),
        }
      );
      ...
      setSt((prev) => {
        if (!prev) return prev;
        const current = prev.convs.find((c) => c.id === activeConv);
        if (!current) return prev;

        const message: ThreadMsg = {
          id: String(serverMessage?.id ?? `local-${Date.now()}`),
          direction: normalizeDirection(serverMessage?.direction ?? "out"),
          body: serverMessage?.body ?? text,
          kind: serverMessage?.kind ?? "manual",
          is_auto: Boolean(serverMessage?.is_auto ?? false),
          created_at: serverMessage?.created_at ?? new Date().toISOString(),
        };

        const exists = current.msgs.some((m) => m.id === message.id);
        const msgs = exists ? current.msgs : [...current.msgs, message];

        const updated = {
          ...current,
          lastAt: message.created_at,
          lastMessagePreview: message.body,
          transferred: resumeAuto ? false : current.transferred,
          paused: resumeAuto ? null : current.paused,
          msgs,
        };
        ...
        return {
          ...
          convs: [
            updated,
            ...prev.convs.filter((c) => c.id !== activeConv),
          ],
        };
      });
                    ...
                    <li key={c.id}>
                      <button
                        onClick={() => {
                          ...
                          setSt((prev) => prev
                            ? { ...prev, convs: prev.convs.map((item) => item.id === c.id ? { ...item, unreadCount: 0 } : item) }
                            : prev
                          );
                        }}
                      ...
                      >
                        ...
                        <p className="text-[11.5px] text-sage truncate">{c.lastMessagePreview || c.msgs[c.msgs.length - 1]?.body || "لا توجد رسائل بعد"}</p>
