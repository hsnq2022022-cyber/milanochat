...
export default function Dashboard() {
  ...
  /* شبكة أمان: تحديث خفيف لقائمة المحادثات (آخر رسالة + الترتيب) كل بضع ثوانٍ
     بشكل مستقل عن Supabase Realtime — حتى تنتقل المحادثة للأعلى وتظهر آخر
     رسالة في القائمة الجانبية فورًا حتى لو لم يصل حدث Realtime لأي سبب.
     لا يمس msgs الخاصة بالمحادثة المفتوحة إطلاقًا. */
    ...
    try {
      ...
      setSt((prev) => {
        ...
        const merged = convs.map((c: any) => {
          const nowD = new Date();
          const expiresAt = c.humanAgentExpiresAt ? new Date(c.humanAgentExpiresAt) : null;
          const humanAgentActive = c.transferred && expiresAt && expiresAt > nowD;
          const remainingSeconds = humanAgentActive && expiresAt ? Math.floor((expiresAt.getTime() - nowD.getTime()) / 1000) : 0;
          const existing = byId.get(c.id);
          const isNewLastMsg = !existing || existing.lastAt !== c.lastMessageAt;
          return {
            ...(existing ?? { msgs: [] }),
            id: c.id,
            phone: c.customerPhone,
            ...
            lastMessagePreview: c.lastMessageBody || "—",
            unreadCount:
              activeConv === c.id || !isNewLastMsg
                ? existing?.unreadCount || 0
                : (existing?.unreadCount || 0) + 1,
          };
        });
        merged.sort((a, b) => new Date(b.lastAt || 0).getTime() - new Date(a.lastAt || 0).getTime());
        return { ...prev, convs: merged };
      });
    } catch (e) {
      console.error("[Dashboard] refreshConvList error:", e);
    }
  }, [token, activeConv]);

  useEffect(() => {
    if (demo || !token || needClaim) return;
    const iv = window.setInterval(() => refreshConvList().catch(() => {}), 4000);
    return () => window.clearInterval(iv);
  }, [demo, token, needClaim, refreshConvList]);
    ...
    try {
      if (demo) {
        ...
        setSt((prev) => {
          ...
          // 1. تحديث بيانات المحادثة: معاينة آخر رسالة وحالة الرد الآلي
          const updatedActiveConv = {
            ...
            // 2. إضافة الرسالة الجديدة فعليًا لمصفوفة رسائل المحادثة المفتوحة
            // (كانت تُحسب ولا تُستخدم — وهذا سبب عدم ظهور الرسالة فورًا)
            msgs: [...(activeConvData.msgs || []), {
              id: `temp-${Date.now()}`,
          ...
          return {
            ...prev,
            convs: prev.convs
              .map(c => c.id === activeConv ? updatedActiveConv : c)
