...
export default function Dashboard() {
  ...
  /* شبكة أمان: تحديث خفيف لقائمة المحادثات (آخر رسالة + الترتيب) كل بضع ثوانٍ
     بشكل مستقل عن Supabase Realtime — حتى تظهر الرسالة الجديدة وتنتقل
     المحادثة للأعلى فورًا حتى لو لم يصل حدث Realtime لأي سبب.
  ...
  const refreshConvList = useCallback(async () => {
    ...
    try {
      ...
      setSt((prev) => {
        ...
        // الحفاظ على ترتيب آخر رسالة (الأحدث أولًا) — الخادم يرتبها بالفعل، لكن للأمان
